...
void	insertVertex(Graph *, VertexNode *);//임의추가
...
EdgeNode	*newEdge(Vertex* , Vertex*);//vertex1, vertex2 묶고 vertexnode생성 하고, 각 vertex에 edgelist에도 추가.  graph에 추가만 하면 됨. 

void deleteEdgeNode(EdgeNode *, int);

//EdgeNode* findEdgeNode(EdgeNode*, Edge*);

void	insertEdge(Graph *, EdgeNode *);//임의추가. EdgeNode list에 Edge를 추가.
...
void insertEdgeToList(Edge *, EdgeNode*);
